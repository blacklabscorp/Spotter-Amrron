...
flMsgWorkingDir = UserProfile+'//.nbems//ICS//messages//'
...
def genspotrepv3_0(spotFileFormat,dataPath):
    global newL01_sp
    global newL02_sp
    global newL03_sp
    global newL04_sp
...
# first writing the HTML form then creating the Last Json
def gensitrepv3_0(sitFileFormat,dataPath):
    global newL01_sr
    global newL21_sr
    global newL03_sr
    global newL04_sr
...
def spotrepv3_0load(spotFileFormat,dataPath):
    target = str(dataPath)
...
def sitrepv3_0load(sitFileFormat,dataPath):
    target = str(dataPath)
...
def flmsgout_amrronv3_0_main():
    ...
    if report in ['spotv30','sitv30']:
        if report=='spotv30':
           ...
           spotrepv3_0load(spotFileFormat,dataPath)
           genspotrepv3_0(spotFileFormat,dataPath)
           shutil.copy2((dataPath+amrronSpotReportNew),(flMsgWorkingDir+amrronSpotReportNew))
        if report=='sitv30':
            sitFileFormat=(timestampFlMsg+newL03_sr+'-'+stateDefault+'-'+callsignPrefix+'-000.k2s')
            sitrepv3_0load(sitFileFormat,dataPath)
            gensitrepv3_0(sitFileFormat,dataPath)
            shutil.copy((dataPath+amrronSitReportNew),(flMsgWorkingDir+amrronSitReportNew))
